...
class Parser {
  ...
  private parseParameters(): Partial<SymbolSpec> {
    ...
    while (this.currentToken().type !== TokenType.RBRACKET && this.currentToken().type !== TokenType.EOF) {
      ...
      switch (key) {
        ...
        case 'sp': {
          // Parallelogram: sp="angle,force" or sp="angle force"
          const spParts = value.replace(/\s+/g, ',').split(',').map(v => parseFloat(v.replace('°', '')));
          if (spParts.length < 2 || spParts.some(isNaN)) {
            throw new Error(`Invalid sp: "${value}" (expected "angle,force")`);
          }
          params.sp = { angle: spParts[0], force: spParts[1] };
          break;
        }
        case 'st': {
          // Trapezoid: st="angle,force" or st="angle force"
          const stParts = value.replace(/\s+/g, ',').split(',').map(v => parseFloat(v.replace('°', '')));
          if (stParts.length < 2 || stParts.some(isNaN)) {
            throw new Error(`Invalid st: "${value}" (expected "angle,force")`);
          }
          params.st = { angle: stParts[0], force: stParts[1] };
          break;
        }
        default:
          break;
  ...
  private parseSymbol(gridWidth: number, gridHeight: number): SymbolSpec {
    ...
    // Handle reference tokens: #id, @name, .class
    if (token.type === TokenType.HASH_REF) {
      refId = token.value;
      char = '#' + token.value; // display placeholder
      this.advance();
    ...
    // Merge ref fields from params if they contain #, @, . references
...
export function stringifySpec(spec: UniCompSpec): string {
  ...
  const symbolsPart = spec.symbols.map(sym => {
    ...
    // Handle reference symbols
    if (sym.refId) {
      return `#${sym.refId}${params}${sym.start}-${sym.end}`;
    }
    if (sym.refName) {
      return `@${sym.refName}${params}${sym.start}-${sym.end}`;
    }
    if (sym.refClass) {
      return `.${sym.refClass}${params}${sym.start}-${sym.end}`;
    }
