...
import { UniCompSpec, getRegistry } from '@/lib/unicomp-parser';
...
function drawSpecOnCanvas(
  ...
  function drawNested(nestedSpec: UniCompSpec, targetX: number, targetY: number, targetW: number, targetH: number, parentFlip?: 'h' | 'v' | 'hv', parentRotate?: number, parentAlpha?: number) {
    const nCellW = targetW / nestedSpec.gridWidth;
    const nCellH = targetH / nestedSpec.gridHeight;

    ctx.save();
    ctx.translate(targetX + targetW / 2, targetY + targetH / 2);
    if (parentFlip) {
      const fx = parentFlip === 'h' || parentFlip === 'hv' ? -1 : 1;
      const fy = parentFlip === 'v' || parentFlip === 'hv' ? -1 : 1;
      ctx.scale(fx, fy);
    }
    if (parentRotate) ctx.rotate((parentRotate * Math.PI) / 180);
    ctx.translate(-(targetW / 2), -(targetH / 2));
    if (parentAlpha !== undefined) ctx.globalAlpha = parentAlpha;

    nestedSpec.symbols.forEach((ns) => {
      const nr = getRect(ns.start, ns.end, nestedSpec.gridWidth);
      const nx = nr.x1 * nCellW;
      const ny = nr.y1 * nCellH;
      const nw = (nr.x2 - nr.x1 + 1) * nCellW;
      const nh = (nr.y2 - nr.y1 + 1) * nCellH;

      const nestedEntry = registry.resolve(ns);
      if (nestedEntry) {
        drawNested(nestedEntry.spec, nx, ny, nw, nh, ns.flip, ns.rotate, ns.opacity);
        return;
      }

      const sx = ns.scale?.x ?? 1;
      const sy = ns.scale?.y ?? 1;
      const fs = Math.min(nw * sx, nh * sy) * 0.85;
      ctx.font = `${fs}px ${ns.fontFamily || 'Inter, system-ui'}`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.globalAlpha = ns.opacity ?? (parentAlpha ?? 1);
      ctx.fillStyle = resolveColor(ns.color, 'hsl(210, 20%, 92%)');
      ctx.save();
      ctx.translate(nx + nw / 2, ny + nh / 2);
      if (ns.flip) {
        const fx = ns.flip === 'h' || ns.flip === 'hv' ? -1 : 1;
        const fy = ns.flip === 'v' || ns.flip === 'hv' ? -1 : 1;
        ctx.scale(fx, fy);
      }
      if (ns.rotate) ctx.rotate((ns.rotate * Math.PI) / 180);
      ctx.fillText(ns.char, 0, 0);
      ctx.restore();
    });
  ...
  spec.symbols.forEach((symbol) => {
    ...
    if (entry) {
      drawNested(entry.spec, x1, y1, width, height, symbol.flip, symbol.rotate, symbol.opacity);
      return;
    }
