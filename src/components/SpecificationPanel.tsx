...
import { resolveColor, renderSpecToOffscreen, applySymbolTransforms } from '@/lib/render-utils';
...
const NestedSymbolPreview: React.FC<{ symbol: SymbolSpec; size?: number }> = ({ symbol, size = 24 }) => {
  ...
  useEffect(() => {
    ...
    if (symbol.flip) {
      ctx.scale(symbol.flip === 'h' || symbol.flip === 'hv' ? -1 : 1, symbol.flip === 'v' || symbol.flip === 'hv' ? -1 : 1);
    }
